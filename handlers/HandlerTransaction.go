...
func (handler TransactionHandler) ShowStationTransactions(c *gin.Context) {
	...
	if station_id != "" {
		...
		if station.StationId != "" {	
			...
			handler.db.Raw("SELECT t.id,m.member_id,m.member_fname,m.member_lname,m.member_country_region,m.member_city,m.member_email_address,m.member_mobile,t.station_id,t.date_created,t.date_updated FROM transaction WHERE t.station_id ='"+ station_id +"' AS t INNER JOIN wp_members as w ON t.member_id=m.member_id").Scan(&query)
