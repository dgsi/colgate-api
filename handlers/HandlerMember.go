...
func (handler MemberHandler) Create(c *gin.Context) {
	...
	if member.MemberEmailAddress != "" {
		...
		if err == nil {
			...
			handler.db.Exec("INSERT INTO wp_members(member_id,member_fname,member_lname,member_email_address,member_mobile,member_registration_date) VALUES (?,?,?,?,?,?)",newMemberId,first_name,last_name,email,contact_no,now)
